...
const DonateModal = ({ onClose }) => {
  ...
  const UPI_ID = "your-upi-id@oksbi"; // e.g., makaut@sbi
  const PAYEE_NAME = "MAKAUT Alumni"; // e.g., Aritra Parai
  // ---------------------------------------

  // Construct the UPI Deep Link
  // pa = Payee Address (UPI ID)
  // pn = Payee Name
  // am = Amount (Optional, usually better to let user enter or pre-fill)
  // cu = Currency
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="hidden md:flex flex-col items-center justify-center bg-gray-50 p-4 rounded-xl border border-gray-200">
            <p className="text-sm text-gray-500 mb-3 font-medium flex items-center gap-2">
              <QrCode size={16} /> Scan with Google Pay / PhonePe
            </p>
          ...
          <div className="md:hidden">
            ...
            >
              <Smartphone size={20} /> Open Google Pay
            </a>
            <p className="text-center text-xs text-gray-400 mt-2">Tap to open UPI app directly</p>
          </div>

          {/* Manual Copy Section */}
          <div className="pt-4 border-t border-gray-100">
            <p className="text-xs text-center text-gray-500 mb-2">Or pay manually to UPI ID:</p>
            <div className="flex items-center justify-between bg-gray-100 p-3 rounded-lg border border-gray-200">
              <span className="font-mono text-gray-700 font-medium">{UPI_ID}</span>
              <button 
                onClick={handleCopy}
